"""Hämtar nyheter från Bohuslänningens webbplats via HTML-skrapning."""
...
BASE_URL = "https://www.bohuslaningen.se"
...
class BohuslaningenFetcher:
    """Hämtar nyhetsartiklar från Bohuslänningen via HTML-skrapning."""
    ...
    async def fetch_news(self, client: httpx.AsyncClient) -> list[dict]:
        try:
            ...
            logger.debug("Bohuslänningen ej tillgänglig: %s", exc)
            return []

        encoding = response.encoding or "utf-8"
        html = response.content.decode(encoding, errors="replace")
        items = _parse_articles(html, BASE_URL)
        logger.debug("Bohuslänningen: %d artiklar hittade", len(items))
        return items
